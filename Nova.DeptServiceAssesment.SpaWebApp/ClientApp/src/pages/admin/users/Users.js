...
import { Spinner, Button, Badge, Card, CardBody, CardHeader, Col, Row, Table, Input, InputGroup, InputGroupAddon , FormGroup,
          Modal, ModalHeader, ModalBody, Form, Label  } from 'reactstrap';
...
class Users extends Component {
  ...
  queryData = (searchterm, activePage, itemsPerPage) => {
    DataService.getUsers(searchterm, activePage, itemsPerPage)
      .then(result => {
        this.setState({          
  ...
  onFormSubmit = (e) => {
    ...
    DataService.importUsers(this.state.file)
        .then(result => {
          this.setState({          
            file: null,
          });
  ...
  handleSearch = (event) => {
    ...
    this.setState({ searching: true, users: null });
    this.queryData(this.state.searchTerm, 1, this.state.itemsPerPage);
  ...
  render() {
    ...
    let dataRows =   this.state.users == null ?
                      <tr>
    ...
    const syncbtn = this.state.saving ?
                    ...
                    <Button type="button" size="sm" color="success" className="float-right" onClick={e => this.syncUsers(e)}><i className="fa fa-refresh"></i> Đồng bộ</Button>;
      ...
      <div className="animated fadeIn">
        <Row>
          <Col>
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i> Danh sách nhân viên
              ...
              <CardBody>
                <FormGroup row>
                    <Col xs="9" md="9">
                      <InputGroup>
                        <Input type="text" id="searchTerm" name="searchTerm" placeholder="Tên / Email" 
              ...
              <Row>
                <Col xs="12">
                    ...
                    <FormGroup row>
                        ...
                        <Col xs="12" md="9">
                        <Input type="file" id="file-input" name="file-input"  onChange={e => this.onChangeFile(e)} />
                        </Col>
                    </FormGroup>
