CREATE PROCEDURE SP_REPORT_1(@PeriodId as INT) 
AS
BEGIN
    ...
    (
        SELECT 
			...
			dbo.FN_NUMOFASSIGNEDDEPTS(' + CAST(@PeriodId AS nvarchar) + ', dp.DepartmentTo) as NumOfAssignedDepts,
		...
		GROUP BY dp.DepartmentTo, q.Id, d.Name,DivCode,DivName
    ) t 
    PIVOT(
        SUM(Score)
